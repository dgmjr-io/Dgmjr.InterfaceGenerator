...
public partial class InterfaceGenerator : IIncrementalGenerator
{
    ...
    protected static void Generate(SourceProductionContext context, ImmutableArray<(AttributeData, INamedTypeSymbol, SemanticModel)> values)
    {
        ...
        foreach (var (attributeData, interfaceSymbol, semanticModel) in values)
        {
            ...
            var decompose = attributeData.ConstructorArguments.Length > 1 && (bool)(attributeData.ConstructorArguments[1].Value ?? false);
            var distinctPropertyNames = classToGenerateTheInterfaceFor.GetMembers()
                ...
                .Distinct()
                .ToImmutableArray();
            var propertyDeclarations = classToGenerateTheInterfaceFor
                    .GetMembers()
                    .Where(member => member.Kind is SymbolKind.Property)
                    .OfType<IPropertySymbol>()
                    .Where(p => p.DeclaredAccessibility == Accessibility.Public)
            ...
            var distinctMethodNames = classToGenerateTheInterfaceFor.GetMembers()
            ...
            var allMmethods = classToGenerateTheInterfaceFor.GetMembers()
                    .Where(member => member.Kind is SymbolKind.Method)
                    .OfType<IMethodSymbol>()
                    .Where(m => m.DeclaredAccessibility == Accessibility.Public && m.CanBeReferencedByName);

            var methodDeclarations = distinctMethodNames.ToDictionary(m => m,
                m => Join("\r\n", MethodDeclarationTemplate.Render(
                    allMmethods.Where(m2 => m2.Name == m).Select(m2 => MethodDeclarationTemplate.Render(new MethodDeclarationModel(m2.ReturnType.ToDisplayString(),
                            m2.Name + (m2.IsGenericMethod ? $"<{Join(", ", m2.TypeParameters.Select(tp => tp.Name))}>" : ""),
                            Join(", ", m2.Parameters.Select(p => $"{p.Type.ToDisplayString()} {p.Name}")),
                            m2.IsGenericMethod ? $"{Join("\r\n",
                                m2.TypeParameters.Select(tp =>
                                m2.TypeParameters.Select(tp =>
            ...
            // var methods = classToGenerateTheInterfaceFor
            //         .GetMembers()
            //         .Where(member => member.Kind is SymbolKind.Method)
            //         .OfType<IMethodSymbol>()
            //         .Where(m => m.DeclaredAccessibility == Accessibility.Public && m.CanBeReferencedByName)
            ...
            var methodsAndProperties = methods.Concat(properties).ToDictionary(kvp => kvp.Key, kvp => kvp.Value);

            if (decompose)
            {
                foreach (var memberDeclaration in methodsAndProperties)
                {
                    context.AddSource($"{interfaceName}.{memberDeclaration.Key}.g.cs",
                        InterfaceDeclarationTemplate.Render(new InterfaceGeneratorModel(interfaceNamespace, interfaceName + memberDeclaration.Key,
                            memberDeclaration.Value)));
                }

                var composedInterfaceName = $"{interfaceName}:{Join(", ", methodsAndProperties.Select(m => $"{interfaceName}m.Key))}";

                context.AddSource($"{interfaceName}.g.cs",
                    InterfaceDeclarationTemplate.Render(new InterfaceGeneratorModel(interfaceNamespace, composedInterfaceName,
                        "")));
            }
            else
            {
                ...
                InterfaceDeclarationTemplate.Render(new InterfaceGeneratorModel(interfaceNamespace, interfaceName,
                    Join("\r\n", methodsAndProperties.Select(m => m.Value)))));
            }
        }
    }
